...
class PreEtsReport extends Component {
  ...
  @action.bound async handleSubmit(studentFilters) {
    ...
    const fileName = getReportFileName('pre-ets-activities', {
      startYear,
      startTerm,
      filtered
    });

    this.submitTask = store.downloadReport('/preEts', fileName, {
      startYearId,
      startTermId,
      ...studentFilters
    });
  ...
  render() {
    ...
    const { startYear, startTerm, canRun, submitTask } = this;

    const startTermName = startYear ? startYear.capitalizedTermType : 'Term';

    return (
      <ReportFormContainer
        title="Pre-ETS Activities"
        titleActiveFilter="Pre-ETS Activities Report - Filtered Students"
        subtitle={`one ${startTermName.toLowerCase()}`}
