...
class TrackToGraduateReport extends Component {
  ...
  @action.bound async handleSubmit(studentFilters) {
    ...
    const fileName = getReportFileName('on-track-to-graduate', { 
      startYear,
      startTerm,
      filtered
    });

    this.submitTask = store.downloadReport(  // PROCESS REPORT 
      '/trackToGraduate',
      fileName,
  ...
  render () {
    ...
    return (
      <ReportFormContainer title="Track to Graduation Report  - All Students" titleActiveFilter="Track to Graduation Report - Filtered Students" subtitle={`one ${startTermName.toLowerCase()}`} onSubmit={this.handleSubmit} closePath={closePath} canRun={canRun} submitTask={submitTask} includeFilters={true}>
...
export default TrackToGraduateReport;